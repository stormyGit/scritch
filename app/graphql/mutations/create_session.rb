    ...
    user = User.find_or_create_by(telegram_id: params[:telegram_id]) do |user|
      ...
      begin
        ...
        if profile_photo_file.present?
          profile_photo_file_path = Telegram.bots[:login].get_file(file_id: profile_photo_file['file_id'])["result"]["file_path"]

          user.remote_avatar_url = "https://api.telegram.org/file/bot#{Telegram.bots[:login].token}/#{profile_photo_file_path}"
        end
