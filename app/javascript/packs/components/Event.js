...
class Event extends React.Component {
  ...
  render() {
    ...
    return (
      <Query query={LOAD_EVENT} variables={{ id: match.params.id.match(/[\w]{8}(-[\w]{4}){3}-[\w]{12}$/)[0] }}>
        {({ loading, error, data }) => {
                  ...
                  <Grid item lg={3} xs={12}>
                    <div className={classes.pictureInfo}>
                      <Grid container spacing={8} justify="space-between" wrap="nowrap">
                        <Grid item>
                          <Typography gutterBottom variant="h6" component="h2" color="secondary" className={classes.eventTitle} noWrap>
                            {event.name}
                          </Typography>
                          <Typography gutterBottom variant="h5" component="h2" className={classes.eventTitle} noWrap>
                            {event.name}
                          </Typography>
                        </Grid>
                        ...
                        <Grid item style={{ flexShrink: 0 }}>
                              <React.Fragment>
                                <SocialButton
                                  ...
                                  params={{
                                    text: `${event.name} via @${process.env.TWITTER_ACCOUNT}`,
                      ...
                      <Grid container spacing={8} alignItems='center' justify="center">
                        <Grid xs={4} item />
                        <Grid xs={4} item>
                          <img src={require('../stormy.jpg')} title={event.name} width='100%' style={{borderRadius: '100%'}}/>
                        </Grid>
                        <Grid xs={4} item />
                      </Grid>
                      <Grid container spacing={8} >
