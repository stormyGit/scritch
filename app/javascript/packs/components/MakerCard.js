  ...
  renderHeader() {
    ...
    return (
      <CardHeader
        ...
        title={<Link to={`/${maker.user.slug}`} className={classes.userLink}>{maker.user.name}</Link>}
        subheader={maker.createdAt ? timeAgo.format(dayjs(maker.createdAt).toDate()) : "Under review"}
      />
    );
  }
          ...
          maker.tagList.length === 0 ?
            ...
            maker.tagList.map((tag) => (
              <Chip
                ...
                component={(props) => <Link rel="nofollow" to={`/makers?${queryString.stringify({ q: tag })}`} {...props} />}
              />
            ))
        }
      </CardContent>
  ...
  renderVertical() {
    ...
    return (
      <Card className={classes.card} elevation={0}>
        ...
        <CardActionArea component={(props) => <Link to={`/makers/${maker.slug}-${maker.id}`} {...props} />}>
          {this.renderMedia()}
          {this.renderContent()}
        </CardActionArea>
  ...
  renderHorizontal() {
    ...
    return (
      <Card className={[classes.card, classes.horizontalCard].join(' ')} elevation={0}>
        <CardActionArea component={(props) => <Link to={`/makers/${maker.slug}-${maker.id}`} {...props} />} className={classes.horizontalMediaContainer}>
          {this.renderMedia()}
        </CardActionArea>
        <div className={classes.horizontalContent}>
          {this.renderHeader()}
          <CardActionArea component={(props) => <Link to={`/makers/${maker.slug}-${maker.id}`} {...props} />} className={classes.horizontalInfos}>
            {this.renderContent()}
          </CardActionArea>
          {this.renderTags()}
          {this.renderActions()}
